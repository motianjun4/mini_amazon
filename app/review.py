...
def submitProductReview():
    ...
    if form.validate_on_submit():
        args = request.args
        rid = Review.submit(current_user.id, 2, 0,
                            args['pid'], form.rate.data, form.review.data)
        for i in range(3):
            key = 'image'+str(i)
...
def editProductReview():
    ...
    if form.validate_on_submit():
        args = request.args
        rid = Review.edit(current_user.id, 2, 0,
                          args['pid'], form.rate.data, form.review.data)
        for i in range(3):
...
def submitSellerReview():
    ...
    if form.validate_on_submit():
        args = request.args
        rid = Review.submit(current_user.id, 1,
                            args['sid'], 0, form.rate.data, form.review.data)
        for i in range(3):
            file = request.files['image'+str(i)]
            if file is not None:
                tmp_filepath = f"/tmp/{uuid1()}.jpg"
...
def editSellerReview():
    ...
    if form.validate_on_submit():
        args = request.args
        rid = Review.edit(current_user.id, 1,
                          args['sid'], 0, form.rate.data, form.review.data)
        for i in range(3):
            file = request.files['image'+str(i)]
            if file is not None:
